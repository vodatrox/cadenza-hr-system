"""
Django settings for cadenza_hr project.
"""
import os
...
RAILWAY_ENVIRONMENT = config('RAILWAY_ENVIRONMENT_NAME', default='')
...
WSGI_APPLICATION = 'cadenza_hr.wsgi.application'
...
# ── Security (production) ──────────────────────────────────────────────────
if not DEBUG:
    # Railway handles SSL at the load balancer — do NOT enable SECURE_SSL_REDIRECT
    # or it will cause an infinite redirect loop.
...
# ── Redis Configuration ────────────────────────────────────────────────────
# Railway provides REDIS_URL when you add a Redis plugin
...
NIGERIAN_TAX_CONFIG = {
    'PENSION_RATE': config('PENSION_RATE', default=0.08, cast=float),
    'NHF_RATE': config('NHF_RATE', default=0.025, cast=float),
    'TAX_FREE_ALLOWANCE': config('TAX_FREE_ALLOWANCE', default=200000, cast=int),
    'TAX_BANDS': [
        ...
        {'min': 3200001, 'max': float('inf'), 'rate': 0.24},
    ]
}
